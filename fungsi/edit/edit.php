...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['barang'])) {
        ...
        $data[] = $tgl;
        $data[] = $id;
        $sql = 'UPDATE barang SET id_kategori=1, nama_barang=?, merk=?, 
    ...
    // Cari data barang dari session $_SESSION['penjualan']
    $found = false; // Untuk melacak apakah item ditemukan dalam session
    foreach ($_SESSION['penjualan'] as $key => $item) {
        if ($item['id_barang'] == $id_barang) {
            $found = true;
            // Ambil harga jual dari session yang sudah ada
            $jual = $item['total'] / $item['jumlah'];
            // Cek apakah stok cukup (dianggap data stok dari database)
            $sql_tampil = "select * from barang where barang.id_barang=?";
            $row_tampil = $config -> prepare($sql_tampil);
            $row_tampil -> execute(array($id_barang));
            $hasil = $row_tampil -> fetch();
            ...
            if ($hasil['stok'] > $jumlah) {
                // Update jumlah dan total dalam session
                $_SESSION['penjualan'][$key]['jumlah'] = $jumlah;
                $_SESSION['penjualan'][$key]['total'] = $jual * $jumlah;

                // Redirect kembali ke halaman keranjang
                echo '<script>window.location="../../index.php?page=jual#keranjang"</script>';
            } else {
                // Tampilkan alert jika stok tidak cukup
                echo '<script>alert("Keranjang Melebihi Stok Barang Anda !");
                    window.location="../../index.php?page=jual#keranjang"</script>';
            }
            break;
    ...
    if (!$found) {
        echo '<script>alert("Barang tidak ditemukan di keranjang!"); window.location="../../index.php?page=jual#keranjang"</script>';
    }
}
...
if (!empty($_GET['pesan'])) {
    ...
    foreach ($_SESSION['pemesanan'] as $key => $item) {
        if ($item['id_barang'] == $id_barang) {
            ...
            if ($hasil['stok'] > $jumlah) {
                // Update jumlah dan total dalam session
                $_SESSION['pemesanan'][$key]['jumlah'] = $jumlah;
                $_SESSION['pemesanan'][$key]['total'] = $jual * $jumlah;

                // Redirect kembali ke halaman keranjang
                echo '<script>window.location="../../index.php?page=pesan#keranjang"</script>';
            } else {
                // Tampilkan alert jika stok tidak cukup
                echo '<script>alert("Keranjang Melebihi Stok Barang Anda !");
    ...
    if (!empty($_GET['cari_barang'])) {
		...
		<?php foreach ($hasil1 as $hasil) {?>
			<tr>
				...
				<td>
				<a href="fungsi/tambah/tambah.php?jual=jual&id=<?php echo $hasil['id_barang'];?>&id_kasir=<?php echo $_SESSION['admin']['id_member'];?>&merk=<?php echo $hasil['merk'];?>" 
    ...
    if (!empty($_GET['cari_p'])) {
		...
		<?php foreach ($hasil1 as $hasil) {?>
			<tr>
				...
				<td><?php echo $hasil['harga_jual'];?></td>
				<td>
				<a href="fungsi/tambah/tambah.php?pesan=pesan&id=<?php echo $hasil['id_barang'];?>&id_kasir=<?php echo $_SESSION['admin']['id_member'];?>&merk=<?php echo $hasil['merk'];?>" 
					class="btn btn-success">
					<i class="fa fa-shopping-cart"></i></a></td>
			</tr>
		<?php }?>
